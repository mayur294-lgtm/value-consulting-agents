...
evals/
  ...
  rubrics/
    ...
    deliverable/
      ...
      visual_render.py           DELETED   D9 forbids a metered vision call as a blocking row + no non-gating deliverable-judge slot exists; had no calibration of its own — see :16
...
1. **`visual_render.py` — wire or delete (:16). RESOLVED: deleted.** The deletion stands. **Correction (v7 PR3):** an earlier record of this resolution claimed reproduction "was not possible" without `ANTHROPIC_API_KEY`. That was false and has been corrected — reproduction was possible and was done, at $0, with `python3 evals/run_experiment.py --deliverable deck --negatives`: golden `evals/goldens/deck_valid_min.html` -> 1.000, negative `evals/goldens/negatives/deck_offpalette.html` -> 0.364 HARD-FAIL. Those figures belong to `decks.py`'s 7-check code rubric (`(0 + 0.90 + 0 + 0 + 0.80 + 0 + 0.85) / 7 = 0.3642857...`, three hard-fails), not to `visual_render.py`'s vision judge, which returns a single raw float and never had a calibration of its own recorded anywhere in-tree — there was nothing of `visual_render.py`'s to reproduce or fail to reproduce. The deletion itself rests on independent grounds: D9 forbids a blocking CI row needing a metered vision-API key; `deliverables:` has no non-gating judge slot — `path1_judge:` is the only such mechanism and it is `components:`-only (tied to `path1.py --agent` regeneration, not to scoring an already-rendered file); the module's own skip paths used the vacuous `passed=True, skipped=True` shape `#181` eliminated elsewhere; and `decks.py` already covers the deck deliverable at $0 with a working negative. Inventing a `deliverable`-altitude non-gating judge is new registry machinery, out of scope here. Deleted the module and its now-orphaned `judge/prompts/visual_render.md`. The deck calibration figures above are now recorded in `evals/rubrics/deliverable/decks.py`'s module docstring, next to the checks that produce them.
