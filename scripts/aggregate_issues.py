...
def queue_for_harness(repo: str, issue_url: str, dry_run: bool = False):
    """Label the top issue 'needs-bb-prd' — queue it for the bb-* harness backlog.

    The old auto-dev loop is gone; NOTHING auto-implements. This just flags the
    highest-priority telemetry finding so a human/agent picks it up through
    bb-prd -> bb-design -> bb-tickets -> bb-build (eval verify) -> bb-pr-review -> bb-refine.
    """
    if dry_run:
        print(f'[DRY RUN] Would label {issue_url} as needs-bb-prd')
        return
...
def main():
    ...
    # Queue the #1 issue for the bb-* harness backlog (no auto-implement)
    if created_urls and created_urls[0] != 'dry-run':
        queue_for_harness(args.repo, created_urls[0], dry_run=args.dry_run)
