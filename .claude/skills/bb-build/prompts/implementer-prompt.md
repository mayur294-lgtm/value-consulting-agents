...
> 3. **Verify** — run the verification commands from the acceptance criteria: `python scripts/test_agent.py` (structural) and `python evals/run_experiment.py --component <component> --altitude unit` (and `--altitude pipeline` where the ticket touches a chained agent).
