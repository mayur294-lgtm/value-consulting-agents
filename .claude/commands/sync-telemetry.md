...
```bash
python3 scripts/extract_telemetry.py [journal_path] --output /tmp/telemetry_extract.json
```
...
gh issue create \
  ...
  --label "telemetry" \
  --body "$(python3 scripts/format_telemetry_issue.py --input /tmp/telemetry_extract.json)"
```
